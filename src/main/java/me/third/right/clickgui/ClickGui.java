...
import me.third.right.clickgui.Settings.Components.HackButton;
...
public final class ClickGui {
	...
	private float[] bonColor = new float[3];
	...
	public void init(HackList hax) {
		//Map and create Windows.
		final LinkedHashMap<Category, Window> windowMap = new LinkedHashMap<>();
		for(Category category : Category.values())
			windowMap.put(category, new Window(category.getName()));

		final ArrayList<Hack> hacks = new ArrayList<>(hax.getValues()).stream().sorted(Comparator.comparing(Hack::getName)).collect(Collectors.toCollection(ArrayList::new));
		for(Hack hack : hacks) {
			if(!moduleReleaseCheck(hack.getReleaseState())) continue;
		...
		//Set windows positions (Alternative if and when there are no config file found.)
		int x = 5;
		int y = 5;
		...
		for(Window window : windows) {
			final JsonElement jsonWindow = json.get(window.getTitle());
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton) {
		final boolean popupClicked = handlePopupMouseClick(mouseX, mouseY, mouseButton);
	...
	private void handleTitleBarMouseClick(Window window, int mouseX, int mouseY, int mouseButton) {
		...
		if(mouseY < window.getY() + 2 || mouseY >= window.getY() + 11) {
			window.startDragging(mouseX, mouseY);return;
		}
	...
	private void handleComponentMouseClick(Window window, int mouseX, int mouseY, int mouseButton) {
		for(int i2 = window.countChildren() - 1; i2 >= 0; i2--)
		{
			...
			if(mouseX < c.getX() || mouseY < c.getY()) continue;
			if(mouseX >= c.getX() + c.getWidth() || mouseY >= c.getY() + c.getHeight()) continue;
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		for(Window window : windows) {
			...
			else if(window.getY() + window.getHeight() > sr.getScaledHeight()) window.setY(sr.getScaledHeight() - window.getHeight());

			if(window.getX() < 0) window.setX(0);
			else if(window.getX() + window.getWidth() > sr.getScaledWidth()) window.setX(sr.getScaledWidth() - window.getWidth());
		...
		if(tooltip != null && !tooltip.isEmpty()) {
			final String[] lines = tooltip.split("\n");
			...
			int tw = 0;
			int th = lines.length * (customFont ? cfr.getHeight() : fr.FONT_HEIGHT);
			for(String line : lines) {
				int lw = (customFont ? cfr.getStringWidth(line) : fr.getStringWidth(line));
				if(lw > tw) tw = lw;
			}
			...
			for(int i = 0; i < lines.length; i++) {
				FontDrawing.drawString(lines[i], xt1 + 2, yt1 + 2 + i * cfr.getHeight(), 0xffffff);
			}
		}
	...
	public void updateColors() {
		...
		maxHeight = hax.getMaxHeight();
		acColor = hax.getAcColor();
		bonColor = hax.getBONColor();
	...
	private void renderWindow(Window window, int mouseX, int mouseY, float partialTicks) {
		...
		if(window.isMinimized()) y2 = y3;
		if(mouseX >= x1 && mouseY >= y1 && mouseX < x2 && mouseY < y2) tooltip = null;
		...
		if(!window.isMinimized()) {
			...
			for(int i = 0; i < window.countChildren(); i++) {
				final Component component = window.getChild(i);
				if (component != null)
					component.render(cMouseX, cMouseY, partialTicks);
			}
			
			GL11.glPopMatrix();
			if(window.isScrollingEnabled()) GL11.glDisable(GL11.GL_SCISSOR_TEST);
		}
		...
		final String title;
		if(ThirdMod.hax.clickGuiHack.customFont.isChecked()) {
			title = window.getTitle();
		} else {
			final FontRenderer fontRenderer = Wrapper.getFontRenderer();
			title = fontRenderer.trimStringToWidth(window.getTitle(), x3 - x1);
		}
	...
	public float[] getAcColor() { return acColor; }
	public float[] getBONColor() { return bonColor; }
	public float[] getOpColour() { return opColour; }
	...
	public static boolean moduleReleaseCheck(final ReleaseType releaseType) {
		final ReleaseType current = ThirdMod.releaseType;
		switch (current) {
			default:
			case Release:
				return releaseType.equals(ReleaseType.Release);
			case PreRelease:
				return releaseType.equals(ReleaseType.Release) || releaseType.equals(ReleaseType.PreRelease);
			case Development:
				return releaseType.equals(ReleaseType.Release) || releaseType.equals(ReleaseType.PreRelease) || releaseType.equals(ReleaseType.Development);
		}
	}

	public void addHack(Hack hack) {
		for(Window window : windows) {
			if(window.getTitle().equals(hack.getCategory().getName())) {
				window.add(new HackButton(hack));
			}
		}
	}
}
